...
(function() {
  var addContact, addListener, clearElement, getPosition, getProperties, getSize, handleDropContactEvent, removeCardContact, setBackgroundColor, setFontColor, setFontSize, setPosition, setProperties, setSize, setText;
    ...
    $.ajax({
      ...
      success: function(json, headers, status) {
        ...
        for (i = j = 0, ref = col; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
          ...
          results.push(console.log('success'));
        }
        return results;
      },
  ...
  handleDropContactEvent = function(event, ui) {
    ...
    return addListener($contact);
  };

  addListener = function($contact) {
    $contact.draggable({
      containment: '.card',
      cursor: 'move'
    });
